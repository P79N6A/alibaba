...
class GenerateShareLinkAlibaba extends React.Component {
  ...
  render() {
    ...
    if (this.state.sharedLinkInfo) {
      ...
      return (
        <div>
          <p className="text-secondary font-weight-normal">{isZHCN ? '查看链接：阿里巴巴经济体内获取该链接的任何人可以查看该文件' : 'View Link: Within Alibaba Group, anyone who gets the link can VIEW the file'}</p>
          <Form className="mb-4">
      ...
      if (fileInfo.name.lastIndexOf('.') != -1) {
        file_ext = fileInfo.name.substr(fileInfo.name.lastIndexOf('.') + 1).toLowerCase();
      }
      return (
        (!fileInfo.can_edit && !fileInfo.can_preview) ?
          ...
          <Form className="generate-share-link">
            ...
            <FormGroup check className="permission">
              <Label check>
                <Input type="radio" name="radio1" defaultChecked={true} onChange={() => this.setPermission('previewOnCloud')}/>{'  '}{gettext('Preview-on-Cloud')}
              </Label>
            </FormGroup>}
            {(fileInfo.can_edit && (file_ext == 'docx' || file_ext == 'xlsx' || file_ext == 'pptx')) &&
            <FormGroup check className="permission">
              <Label check>
                <Input type="radio" name="radio1" onChange={() => this.setPermission('editOnCloud')} />{'  '}{gettext('Preview-Edit-on-Cloud')}
              </Label>
            </FormGroup>}
